...
#SBATCH --output=logs/par-rl1-%j.log
#SBATCH --error=logs/par-rl1-%j.error
...
MODELS=(gemma2-2b gemma2-9b llama-3.1-8b qwen3-4b-base)
DATASETS=(hh_rlhf ultrafb_bin)
ALGOS=(ppo a2c)
# SHAPINGS=(vanilla warm meanstd par)
SHAPINGS=(vanilla par)
PIPELINE_NAME=02_rl_matrix_part1
...
declare -A _HP_POLICY_LR=(
  ["gemma2-2b:hh_rlhf"]="${GEMMA2_2B_RL_LR:-6e-7}"              ["gemma2-2b:ultrafb_bin"]="${GEMMA2_2B_RL_LR:-1e-6}"
  ["gemma2-9b:hh_rlhf"]="${GEMMA2_9B_RL_LR:-3e-7}"              ["gemma2-9b:ultrafb_bin"]="${GEMMA2_9B_RL_LR:-5e-7}"
  ["qwen3-4b-base:hh_rlhf"]="${QWEN3_4B_BASE_RL_LR:-2e-7}"      ["qwen3-4b-base:ultrafb_bin"]="${QWEN3_4B_BASE_RL_LR:-5e-6}"
  ["llama-3.1-8b:hh_rlhf"]="${LLAMA3_1_8B_RL_LR:-2e-7}"         ["llama-3.1-8b:ultrafb_bin"]="${LLAMA3_1_8B_RL_LR:-2e-6}"
)
declare -A _HP_CRITIC_LR=(
  ["gemma2-2b:hh_rlhf"]="${GEMMA2_2B_CRITIC_LR:-6e-6}"          ["gemma2-2b:ultrafb_bin"]="${GEMMA2_2B_CRITIC_LR:-1e-5}"
  ["gemma2-9b:hh_rlhf"]="${GEMMA2_9B_CRITIC_LR:-3e-6}"         ["gemma2-9b:ultrafb_bin"]="${GEMMA2_9B_CRITIC_LR:-5e-6}"
  ["qwen3-4b-base:hh_rlhf"]="${QWEN3_4B_BASE_CRITIC_LR:-2e-6}"  ["qwen3-4b-base:ultrafb_bin"]="${QWEN3_4B_BASE_CRITIC_LR:-5e-5}"
  ["llama-3.1-8b:hh_rlhf"]="${LLAMA3_1_8B_CRITIC_LR:-2e-6}"     ["llama-3.1-8b:ultrafb_bin"]="${LLAMA3_1_8B_CRITIC_LR:-2e-5}"
)
declare -A _HP_EPOCHS=(
  ["gemma2-2b:hh_rlhf"]="0.5"                                    ["gemma2-2b:ultrafb_bin"]="0.5"
...
declare -A _HP_BUFFER_SIZE=(
  ["gemma2-2b:hh_rlhf"]="${GEMMA_RL_BUFFER_SIZE:-${GEMMA_PPO_BUFFER_SIZE:-8}}"                         ["gemma2-2b:ultrafb_bin"]="${GEMMA_RL_BUFFER_SIZE:-${GEMMA_PPO_BUFFER_SIZE:-8}}"
  ["gemma2-9b:hh_rlhf"]="${GEMMA2_9B_RL_BUFFER_SIZE:-${GEMMA2_9B_PPO_BUFFER_SIZE:-8}}"                 ["gemma2-9b:ultrafb_bin"]="${GEMMA2_9B_RL_BUFFER_SIZE:-${GEMMA2_9B_PPO_BUFFER_SIZE:-8}}"
  ["qwen3-4b-base:hh_rlhf"]="${QWEN3_4B_BASE_RL_BUFFER_SIZE:-${QWEN3_4B_BASE_PPO_BUFFER_SIZE:-8}}"     ["qwen3-4b-base:ultrafb_bin"]="${QWEN3_4B_BASE_RL_BUFFER_SIZE:-${QWEN3_4B_BASE_PPO_BUFFER_SIZE:-8}}"
  ["llama-3.1-8b:hh_rlhf"]="${LLAMA3_1_8B_RL_BUFFER_SIZE:-${LLAMA3_1_8B_PPO_BUFFER_SIZE:-8}}"         ["llama-3.1-8b:ultrafb_bin"]="${LLAMA3_1_8B_RL_BUFFER_SIZE:-${LLAMA3_1_8B_PPO_BUFFER_SIZE:-8}}"
)
...
for model in "${MODELS[@]}"; do
  for dataset in "${DATASETS[@]}"; do
    for algo in "${ALGOS[@]}"; do
      ...
      if [[ "$algo" == "a2c" ]]; then
        policy_learning_rate=$(awk "BEGIN {printf \"%.2e\", $policy_learning_rate * 0.5}")
        critic_learning_rate=$(awk "BEGIN {printf \"%.2e\", $critic_learning_rate * 0.5}")
      fi
      ...
      echo "[PLAN] objective=${algo} model=${model} dataset=${dataset} train_batch_size_per_gpu=${train_batch_size_per_gpu} eval_batch_size_per_gpu=${eval_batch_size_per_gpu} KL_coef=${kl_coef} skip_initial_eval_ontest=${skip_initial_eval_ontest} rollout_max_batch_prompts=${rollout_max_batch_prompts} rollout_max_batch_tokens=${rollout_max_batch_tokens}"
