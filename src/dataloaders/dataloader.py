...
class DataLoader:
    ...
    def __init__(self, 
        ...
        if self.batch_size < self.world_size:
            raise ValueError(
                f"Global batch size {self.batch_size} must be >= world_size {self.world_size}"
            )
        if self.batch_size % self.world_size != 0:
            raise ValueError(
                f"Global batch size {self.batch_size} must be divisible by world_size {self.world_size} "
                "for distributed dataloading."
            )
        self.n_data_per_gpu = self.batch_size // self.world_size
    ...
    def split_batch_for_gpus(self, batch):
        """
        Split a global batch into equal per-rank microbatches.
        We require exact divisibility here so every rank executes the exact same
        number of steps and enters the same collectives with the same local shapes.
        """
        num_samples = len(batch)
        if num_samples % self.world_size != 0:
            raise ValueError(
                f"Batch with {num_samples} samples is not divisible by world_size={self.world_size}. "
                "This would desynchronize distributed collectives."
            )
        samples_per_gpu = num_samples // self.world_size
        if samples_per_gpu == 0:
            raise ValueError(
                f"Batch with {num_samples} samples gives 0 samples per GPU for world_size={self.world_size}."
            )
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for idx, example in enumerate(self.full_data):
                ...
                batch_element['row_index'] = example.row_index
                ...
                batch_element['reference_response'] = example.reference_response
            ...
            if len(batch) > 0:
                if len(batch) % self.world_size == 0:
                    ...
                    self.log_message_rank0(
                        f'Dropping incomplete batch of size {len(batch)} on split={self.split} to preserve distributed sync.'
                    )
...
class SFTDataLoader(DataLoader):
    def __iter__(self):
        ...
        while True:
            ...
            # Drop incomplete global batches. Keeping them would produce different
            # local tensor shapes or different numbers of steps across ranks.
            if len(batch) == self.batch_size:
                example_idx += len(batch)
                batch_for_gpu = self.split_batch_for_gpus(batch)
                yield self.collate(batch_for_gpu)
